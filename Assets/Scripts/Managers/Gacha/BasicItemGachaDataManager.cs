using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using DataTable_FortuneHero;

public class BasicItemGachaDataManager : BasicItemGachaData
{
    public List<KeyValuePair<int, float>> Get()
    {
        List<KeyValuePair<int, float>> newList = new List<KeyValuePair<int, float>>();
        foreach (var item in BasicItemGachaDataList)
        {
            KeyValuePair<int, float> newPair = new KeyValuePair<int, float>(item.ID, item.weight);
            newList.Add(newPair);
        }

        return newList;
    }

    public BasicItemGachaData Get(int id)
    {
        if (BasicItemGachaDataMap.TryGetValue(id, out BasicItemGachaData data))
            return data;
        else
            return null;
    }

    
}
